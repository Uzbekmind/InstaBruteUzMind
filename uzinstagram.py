...
class Engine(object):
    ...
    def passlist_path_exists(self):
        if not exists(self.passlist_path):
            self.display.warning('Parol lugati uchun xato yol korsatilgan')
            return False
        return True
    ...
    def get_user_resp(self):
        return self.display.prompt('Hujumni davom etishni hohlaysizmi? [H/Y]: ')
...
def valid_int(n):
    if not n.isdigit():
        raise ArgumentTypeError('Bolim raqam bilan tanlanishi shart')

    n = int(n)

    if n > 3:
        raise ArgumentTypeError('Eng yuqori bolim raqami 3')

    if n < 0:
        raise ArgumentTypeError('Eng past raqam esa 0')
...
if __name__ == '__main__':
    ...
    if int(python_version()[0]) < 3:
        print('[!] Iltimos Python 3 orqali ishlating')
        exit()
